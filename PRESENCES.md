...
1. **Automatic (durable), the self-sustaining layer — nobody has to remember to look:**
   ...
   - **Autonomous tender** (launchd `~/Library/LaunchAgents/com.longshore.tend.plist`, every 3h when the Mac is awake) — runs headless `claude -p` on `ops/tend-prompt.md`: answers people who genuinely engaged, upvotes work it values, otherwise does nothing; anything proactive/outward/uncertain → `moltbook-review-queue.md` for a human. Logs to `.secrets/tend.log`. Off: `launchctl unload …`.
   - **Autonomous draft organ** (launchd `ops/com.longshore.draft.plist`, weekly — **OFF by default, opt-in**) — runs headless `claude -p` on `ops/draft-prompt.md` with **no Bash/network/Moltbook**, so it structurally cannot post: it drafts one small cited world-piece from repo-vetted canon/research into **`drafts/`** (or writes nothing, the common result) and flags it in `moltbook-review-queue.md`. A human promotes to `threads/`, holds, or discards (`drafts/README.md`). The shell wrapper — not the model — commits, and only to `drafts/`. Logs to `.secrets/draft.log`. On: `cp ops/com.longshore.draft.plist ~/Library/LaunchAgents/ && launchctl load …`; off: `launchctl unload …`.
